...
function deriveHues(name: string, collection: string): Hue[] {
  ...
  // Fallback to grey (neutral) instead of white so unmatched products
  // don't pollute the White hue filter
  return matches.length > 0 ? matches : ["grey"];
}

/* ------------------------------------------------------------------ *
 * Pattern derivation — keyword matching against collection/name       *
 * ------------------------------------------------------------------ */
function derivePattern(name: string, collection: string): Pattern {
  const lower = `${name} ${collection}`.toLowerCase();
...
export function mapSanityToCatalogue(products: SanityProduct[]): Slab[] {
  return products
    ...
    .map((p) => {
      ...
      const hues = deriveHues(p.name, collection);
      const pattern = derivePattern(p.name, collection);
      ...
      return {
        ...
        swatch: HUE_SWATCH[hues[0]] ?? HUE_SWATCH.white,
